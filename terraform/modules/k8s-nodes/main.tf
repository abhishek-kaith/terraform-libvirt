...
resource "libvirt_volume" "volume_qcow2" {
  count          = var.number
  name           = "${var.volume-prefix}_${count.index + 10}.qcow2"
...
resource "libvirt_cloudinit_disk" "commoninit" {
  ...
  pool  = var.pool
  user_data = templatefile("${path.module}/cloud_init/cloud_init-k8s-nodes.cfg", {
    hostname       = "k8s-controllers-${count.index + 10}"
    ssh-public-key = var.ssh-public-key
  })
  network_config = templatefile("${path.module}/cloud_init/network_config-k8s-nodes.cfg", {
